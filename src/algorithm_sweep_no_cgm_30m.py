...
def main():
    ...
    # A small but diverse sweep of standard algorithms
    models = {
        "ridge": Pipeline([
        ...
        "huber": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler()),
            ("model", HuberRegressor(epsilon=1.6, alpha=0.001, max_iter=200)),
        ]),
        "knn": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler()),
        ...
        "hist_gbr": Pipeline([
            ...
            ("model", HistGradientBoostingRegressor(
                learning_rate=0.04, max_depth=8, max_iter=320, min_samples_leaf=60, random_state=42
            )),
        ]),
        "gbr": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("model", GradientBoostingRegressor(
                learning_rate=0.05, n_estimators=240, max_depth=3, subsample=0.8, random_state=42
            )),
        ]),
        "random_forest": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("model", RandomForestRegressor(
                n_estimators=240, max_depth=16, min_samples_leaf=2, n_jobs=-1, random_state=42
            )),
        ]),
        "extra_trees": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("model", ExtraTreesRegressor(
                n_estimators=260, max_depth=20, min_samples_leaf=2, n_jobs=-1, random_state=42
            )),
        ]),
    }
    ...
    for user in users:
        ...
        pool_plus_user = pd.concat(
            [pool_no_user, u_train.dropna(subset=["y_30"])],
            ignore_index=True,
        )
        if len(pool_plus_user) > MAX_POOL_ROWS:
            idx = RNG.choice(pool_plus_user.index.to_numpy(), size=MAX_POOL_ROWS, replace=False)
            pool_plus_user = pool_plus_user.loc[idx]

        for model_name, model in models.items():
            # general no-CGM
            mg = fit_with_optional_weights(model, pool_no_user[features_no_cgm], pool_no_user["y_30"])
            pg = mg.predict(u_test[features_no_cgm])
            rows.append({
                "user_id": user,
                "mode": "general_no_cgm",
                "model": model_name,
                "rmse": rmse(y_test, pg),
                "mae": mean_absolute_error(y_test, pg),
                "n_test": len(y_test),
            })

            # pooled + user (simple personalization no-CGM)
            mp = fit_with_optional_weights(model, pool_plus_user[features_no_cgm], pool_plus_user["y_30"])
            pp = mp.predict(u_test[features_no_cgm])
            rows.append({
                "user_id": user,
                "mode": "pooled_plus_user_no_cgm",
                "model": model_name,
                "rmse": rmse(y_test, pp),
            ...
            # user-only personalization no-CGM
            mu = fit_with_optional_weights(model, u_train[features_no_cgm], u_train["y_30"])
            pu = mu.predict(u_test[features_no_cgm])
            rows.append({
                "user_id": user,
                "mode": "user_only_no_cgm",
                "model": model_name,
                "rmse": rmse(y_test, pu),
                "mae": mean_absolute_error(y_test, pu),
                "n_test": len(y_test),
            })
    ...
    out_csv = results_dir / "algorithm_sweep_no_cgm_results.csv"
    out_json = results_dir / "algorithm_sweep_no_cgm_top_models.json"

    results.to_csv(out_csv, index=False)

    top = (
        summary.sort_values(["mode", "RMSE", "MAE"])
